...
($ => {
  ...
  const bootstrap = {
    3: {
      ...
      icons: {
        ...
        toggleOff: 'glyphicon-ams-alt icon-ams-alt',
        toggleOn: 'glyphicon-ams-alt icon-ams-alt',
  ...
  class BootstrapTable {
    ...
    initPagination () {
      ...
      if (!this.options.onlyInfoPagination) {
        $pageList = this.$pagination.find('.page-ams a')
        ...
        if (this.options.smartDisplay) {
          ...
          if (pageList.length < 2 || this.options.totalRows <= pageList[0]) {
            this.$pagination.find('span.page-ams').hide()
          }
